...
│ ...（生成中...）                             │
│ ─────────────────────────────────────────── │
│ ⏱️ 5m 12s · 🤖 mimo-v2.5 · 🧠 82k/1.0m (8%)│
└─────────────────────────────────────────────┘
```

<img width="976" height="936" alt="图片" src="https://github.com/user-attachments/assets/8b4b84c5-9508-4ddd-897a-91bd2f6a17f3" />
...
│ 📆 本月 50 轮 · 🪙 28.8w · ⚡ 86%          │
│ ─────────────────────────────────────────── │
│ 🪙 1.11w+2345 · ⚡94% · 🧠 131k/1.0m (13%)│
│ ✅ 已完成 · ⏱️ 8m 18s · 🤖 mimo-v2.5       │
└─────────────────────────────────────────────┘
```

<img width="1000" height="1149" alt="图片" src="https://github.com/user-attachments/assets/c8915d72-1fca-46be-ab9c-34ae10ce10ff" />
...
流式输出过程中，卡片底部实时显示：
- ⏱️ 耗时 + 🤖 模型名称 + 🧠 上下文窗口占用（已用/总量 + 百分比）

输出完成后，额外显示完整统计：
- 🪙 Token 消耗（输入+输出）+ ⚡ 缓存命中率 + 🧠 上下文窗口占用
...
| 流式 Footer | ❌ 无 | ✅ 实时显示耗时/模型/上下文 |
| 卡片状态区分 | ❌ 无 header | ✅ 青/绿/红/橙 header |
| 会话统计 | ❌ 无 | ✅ 累计 Token + 缓存命中 |
| 日/月统计 | ❌ 无 | ✅ 按日期/月份聚合 |
| Token 准确性 | ⚠️ 累计值 | ✅ 当前轮独立值 |
| 上下文窗口 | ⚠️ 可能不准确 | ✅ runtime 实时数据 |
| 提问过期 | 5 分钟 | 10 分钟 |
